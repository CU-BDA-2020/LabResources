...
# For controlling parallel jobs under Windows:
...
class StanFitResults:
    ...
    def __init__(self, fitter, stan_fit):
        ...
        # Creat an ArviZ InferenceData object if arviz is present.
        if az:
            self.azdata = az.from_pystan(posterior=self.fit)
        else:
            self.azdata = None
    ...
    def _make_param_handler(self, name=None, indx=None, log_p=False):
        ...
        if indx is None:  # scalar case, including log_p
            ...
            # This worked in PyStan-2.17, where flat names used 0-based indices:
            # s_indx = [str(i) for i in indx]
            # As of PyStan-2.18, flat names for params use 1-base indices.
            s_indx = [str(i+1) for i in indx]
...
class StanFitter:
    ...
    def __init__(self, source, data=None, n_chains=None, n_iter=None,
        ...
        if data is not None and n_chains is not None and n_iter is not None:
            fit = self.sample(data=data, chains=n_chains, iter=n_iter,
                              n_jobs=self.n_jobs, **kwds)
            self.fits = [fit]
            return fit
        else:
            self.fits = None
    ...
    def set_n_jobs(self, n_jobs):
        ...
        if plat_is_win:
            if py_major == '3':
                # This is SUPPOSED to work with PyStan-2.19 & Py-3, but it
                # doesn't as of 2020-04-17:
                # self.n_jobs = n_jobs
                # Instead, force Windows not to use parallel jobs:
                self.n_jobs = 1
            else:
                self.n_jobs = 1
        else:
    ...
    def sample(self, n_iter=None, n_chains=None, data=None, control=None,
               **kwds):
        """
        ...
        this.  The `control` argument can be a dict of parameters controlling
        the adaptivity of Stan's NUTS sampler; see below.

        This skips the model compilation step, but otherwise runs a fresh
        MCMC chain.

        Any additional keyword arguments are passed to PyStan's `sampling()`
        method.

        The optional `control` argument is a dictionary that
        can contain key-value pairs controlling Stan's sampling algorithms.
        This is especially useful for adjusting adaptivity in chains aiming
        to explore a posterior PDF with strongly varying scales.  When the
        default values lead to convergence problems, Stan will warn about
        "divergences" (mismatches between the initial and final values of
        the Hamiltonian in the NUTS algorithm).  Changing control parameters
        can help with this.  See the PyStan API docs for a list of
        control parameters:

            https://pystan.readthedocs.io/en/latest/api.html

        See the discussion of divergences in the Stan Reference Manual, and
        the Brief Guide to Stan’s Warnings:

            https://mc-stan.org/docs/2_23/reference-manual-2_23.pdf
            https://mc-stan.org/misc/warnings.html

        By default, StanFitter sets one of the control parameters,
        `adapt_delta`, to 0.95, higher than Stan's built-in default of 0.8.
        `adapt_delta` is a target acceptance rate (b/t 0 and 1).
        Here StanFitter follows the practice of the rstanarm R regression
        package, opting for more conservative exploration to avoid divergences
        that arise fairly commonly.  See the rstanarm discussion:
